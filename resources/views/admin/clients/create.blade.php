<x-admin titleWindow="Crear Cliente" :breadcrumbs="[
    ...
    <x-card>
        <x-slot-header eyebrow="Gestor de Clientes" title="Crear Cliente"
            description="Agrega un nuevo cliente a tu sistema. Completa los campos requeridos para registrar la información básica del cliente." />
        <form action="{{ route('admin.clients.store') }}" method="POST">
            ...
            <x-tabs :active="$defaultTab">
                ...
                <x-tab-content tab="general-information">
                    <div>
                        <x-input label="Nombre *" name="name" :value="old('name')" required autofocus
                            autocomplete="name" placeholder="Juan Carlos" />
                    </div>
                    <div>
                        <x-input label="Apellido *" name="last_name" :value="old('last_name')" required
                            autocomplete="family-name" placeholder="Garcia Perez" />
                    </div>
                    <div>
                    ...
                    <div>
                        <x-input label="CURP *" name="curp" :value="old('curp')" placeholder="GAPJ850412HDFRRN03"
                            required maxlength="18" class="uppercase" />
                    </div>
                    <div>
                        <x-password label="Contraseña *" type="password" name="password" required
                ...
                <x-tab-content tab="fiscal-information">
                    <div>
                        ...
                        ]" select="label:name|value:id" placeholder="Selecciona el tipo de persona" :value="old('person_type')" />
                    </div>
                    <div>
                        <x-select.styled label="Régimen Fiscal" name="fiscal_regime" :options="[
                            ...
                            select="label:name|value:id" placeholder="Selecciona el régimen fiscal"
                    ...
                    <div>
                        <x-select.styled label="Actividad Económica" name="economic_activity" :options="[
                            ...
                            select="label:name|value:id" placeholder="Selecciona la actividad económica" :value="old('economic_activity')" />
                    </div>
                    <div>
                        <x-select.styled label="Uso de CFDI" name="cfdi_use" :options="[
                            ...
                            select="label:name|value:id" placeholder="Selecciona el uso de CFDI" :value="old('cfdi_use')" />
                    </div>
                    ...
                    <div>
                        <x-input label="Código Postal" name="zip_code" placeholder="Ej. 97000" maxlength="5"
                            x-mask="99999" :value="old('zip_code')" />
                    </div>
                    <div>
                        <x-select.styled label="Estatus" name="status" :options="[
                            ...
                            placeholder="Selecciona el estatus" :value="old('status')" />
                    </div>
            ...
            <div class="mt-6">
                <x-button type="submit" text="Crear Cliente" color="indigo" class="font-semibold text-[12px]" />
            </div>
        </form>
    </x-card>
</x-admin>
